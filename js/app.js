...
const loadProducts = () => {
  const data = storeAndPassDatabase();
  showProducts(data);
};
...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick = "showProductDetailsById(${product.id})">
      Details
    </button>
      `;
...
// Details function 
const showProductDetailsById = id => {
  const products = storeAndPassDatabase();

  const detailsContainer = document.getElementById('detail-modal');

  // console.log(id);
  const targetedProduct = products.find(product => product.id === id);
  console.log(targetedProduct);
  const image = targetedProduct.image;
  const div = document.createElement('div');
  div.innerHTML = `
  ...
  <img class="product-image " src=${image}></img>
    ...
    <h3>${targetedProduct.title}</h3>
    <p>Category: ${targetedProduct.category}</p>
    <p>Description: ${targetedProduct.description}</p>
    <div class="rating-area">
  <div><h5 class="fw-bolder text-info">Rating: ${targetedProduct.rating.rate}</h5></div>
  <div><h5 class="fw-bolder text-info">Count: ${targetedProduct.rating.count}</h5></div>
  </div>
    <h2>Price: $ ${targetedProduct.price}</h2>
...
const searchResults = () => {
  ...
  if (searchInput !== '') {
    const filterdProducts = data.filter(product => product.title.toLowerCase().includes(searchInput.toLowerCase()))
